#!/usr/bin/env python
"""Insert sample crop rows into the database.

Run from backend with DJANGO_SETTINGS_MODULE and DATABASE_URL set.
"""
import os, sys
...
def main():
    ...
    created = 0
    for s in samples:
        if not Crop.objects.filter(name=s['name']).exists():
            Crop.objects.create(**s)
            created += 1

    print('Inserted sample crops:', created)
